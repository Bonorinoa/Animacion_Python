...
class StarWars(ThreeDScene):
    def construct(self):
        ...
        saludo = Text("Hola! Soy Dr. Popper").next_to(penguin, UP).scale(0.5)
...
## Ultimos dos ejemplos de Data Science complejos
## 3D surfaces ()

from test import NeuralNetworkMobject
...
class CountingScene(Scene):
    def construct(self):
        ...
        self.play(Count(number, 10, 0, rate_func=rate_functions.ease_out_sine), run_time=6)

        self.play(Flash(number, line_length=1,
            num_lines=30, flash_radius=2+SMALL_BUFF, time_width=0.3, run_time=2, rate_func = rush_from))


        self.wait()

        textoFinal = Text("Y vos que vas a crear?")

        self.play(ReplacementTransform(number, textoFinal))
